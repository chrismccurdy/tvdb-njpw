...
class SonarrClient:
    ...
    def njpw_search(self, series_id, tvdb_id):
        try:
            ...
            return True, association.njpw_world_episode, association.tvdb_episode
        except:
            ...
            for s, ser in enumerate(series):
                ...
                for e, ep in enumerate(episodes):
                    if ser["id"] == ep["seriesId"]:
                        ...
                        found, njpw_ep, tvdb_ep = self.njpw_search(
                            ser["id"], ep["tvdbId"]
                        )
                        if found:
                            if self.download_episode(tvdb_ep, njpw_ep, ser):
    ...
    def download_episode(self, tvdb_ep, njpw_ep, ser=None):
        downloaded = False

        if ser is None:
            ser = self.get_series_by_series_id(njpw_ep.series.id)
            for wnt in self.series:
                if wnt.title == ser["title"]:
                    # Set default values
                    ser["subtitles"] = False
        ...
        dlurl = njpw_ep.url
        outtmpl = (
            "/sonarr_root{0}/Season {1}/{2} - S{1}E{3} - {4} WEBDL.%(ext)s".format(
                ser["path"],
                tvdb_ep.season,
                njpw_ep.series.title,
                tvdb_ep.episode,
                tvdb_ep.title,
            )
        )
        logger.info(f"downloading [{tvdb_ep.title}] from [{dlurl}] to [{outtmpl}]")
        ytdl_format_options = {
            "quiet": True,
        ...
        try:
            ...
            njpw_ep.save()
            logger.info(f"      Downloaded - [{tvdb_ep.title}]")
            downloaded = True
        except Exception as e:
            logger.error(f"      Failed - [{tvdb_ep.title}] - [{e}]")
        finally:
...
def ytdl_hooks(d):
    ...
    if "filename" in d:
        ...
        status_items = list(
            filter(lambda x: x["filename"] == filename, download_status)
        )

    if status == "downloading":
        fragment_count = d["fragment_count"]
        fragment_index = d["fragment_index"]
        percent_complete = int((fragment_index / fragment_count) * 100)
        # logger.trace(f"     Downloading [{filename}] {percent_complete}%")
        if len(status_items) == 0:
            download_status.append({"filename": filename, "progress": percent_complete})
        else:
            status_items[0]["progress"] = percent_complete
    elif status == "finished":
        logger.info(f"      Downloaded - [{filename}]")
        if len(status_items) != 0:
            download_status.remove(status_items[0])
...
def download(association: Association) -> bool:
    client = SonarrClient()
    return client.download_episode(
        tvdb_ep=association.tvdb_episode, njpw_ep=association.njpw_world_episode
    )
