...
import NjpwWorldSeriesService from '../services/NjpwWorldSeriesService'
...
async function getSeries() {
  await NjpwWorldSeriesService.getAll()
...
function save() {
  ...
  NjpwWorldEpisodeService.update(ep.id, ep)
    .then(() => success.show('Episode saved', 5))
    .catch(error.show)
}

getSeries()
NjpwWorldEpisodeService.get(route.params.id)
...
<template>
  ...
  <div class="container mx-0" v-if="episode">
    ...
    <div class="row my-1">
      <div class="label col-sm-2 my-2"><label for="url">URL</label></div>
      <div class="input col-lg-8">
        <input
          id="url"
          name="url"
          ...
          v-model="episode.url"
          placeholder="NJPW World URL"
        />
      </div>
    </div>

    <div class="row my-1">
      <div class="label col-sm-2 my-2"><label for="downloaded-at">Downloaded At</label></div>
      <div class="input col-lg-8">
        <input
          id="downloaded-at"
          name="downloaded-at"
